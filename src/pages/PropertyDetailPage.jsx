...
export default function PropertyDetailPage() {
  ...
  return <main className="property-detail-page">
    <div className="container detail-top"><Link className="back-link" to={returnTo}><ChevronLeft />{returningToProperty ? 'Back to previous property' : 'Back to results'}</Link><div className="property-title-row"><div><div className="detail-badges"><span>{property.purposeLabel}</span><span>{property.availability}</span>{property.verification.is_fully_verified && <span className="verified"><ShieldCheck />Fully verified</span>}</div><h1>{property.title}</h1><p><MapPin />{property.location}</p></div><div className="detail-price"><small>Asking price</small><strong>{property.priceLabel}</strong>{property.price_per_sqft && <span>{formatPrice(property.price_per_sqft, property.currency)} / sq ft</span>}</div></div></div>
    <div className="container key-facts" aria-label="Key property facts">{facts.map((fact) => <div key={fact.label}><span>{fact.label}</span><strong>{fact.value}</strong></div>)}</div>
    ...
    <div className="container detail-layout"><article className="detail-content">
      ...
      {property.latitude && property.longitude && <DetailSection title="Location"><iframe className="property-map" title={`${property.title} location`} loading="lazy" src={`https://www.openstreetmap.org/export/embed.html?bbox=${Number(property.longitude)-.01}%2C${Number(property.latitude)-.01}%2C${Number(property.longitude)+.01}%2C${Number(property.latitude)+.01}&layer=mapnik&marker=${property.latitude}%2C${property.longitude}`} /></DetailSection>}
...
function detailFacts(property) {
  if (property.propertyType === 'land') return [
    { label: 'Land area', value: formatMeasurement(property.land_area_value, property.land_area_unit) || 'Ask agency' },
    { label: 'Road access', value: property.road_access_value ? `${property.road_access_value} ${property.road_access_unit || ''}` : titleCase(property.road_type) || 'Ask agency' },
    { label: 'Facing', value: property.facing_direction_display || 'Ask agency' },
    { label: 'Classification', value: titleCase(property.land_use_classification) || property.propertyTypeLabel },
  ]
  if (['commercial', 'office_space'].includes(property.propertyType)) return [
    { label: 'Property type', value: property.propertyTypeLabel },
    { label: 'Area', value: property.areaLabel || 'Ask agency' },
    { label: 'Parking', value: property.parking_spaces ? `${property.parking_spaces} spaces` : property.parking_type || 'Ask agency' },
    { label: 'Furnishing', value: property.furnishing_status_display || 'Ask agency' },
  ]
  return [
    { label: 'Bedrooms', value: property.beds || 'Ask agency' },
    { label: 'Bathrooms', value: property.baths || 'Ask agency' },
    { label: 'Area', value: property.areaLabel || 'Ask agency' },
    { label: 'Property type', value: property.propertyTypeLabel },
  ]
}
...
function InquiryPanel({ agency, property }) {
  ...
  async function submit(event) { event.preventDefault(); setState({ status: 'submitting', error: '' }); try { if (mode === 'visit') await requestSiteVisit(agency, property.id, { ...form, preferred_datetime: new Date(form.preferred_datetime).toISOString() }); else await submitPropertyInquiry(agency, property.id, form); setForm({ full_name: '', phone: '', email: '', message: '', preferred_datetime: '' }); setState({ status: 'success', error: '' }) } catch (error) { setState({ status: 'error', error: error.message }) } }
  return <aside className="inquiry-panel" id="property-inquiry">
    ...
    <h2>Interested in this property?</h2><p>Contact the agency directly or send an inquiry.</p>
    ...
    {state.status === 'success' ? <div className="form-success"><ShieldCheck /><h3>Request received.</h3><p>The {agency.name} team will contact you soon.</p><button className="text-link" onClick={() => setState({ status: 'idle', error: '' })}>Send another request</button></div> : <>
      ...
      <form onSubmit={submit}><label>Full name<input value={form.full_name} onChange={(event) => set('full_name', event.target.value)} required /></label><label>Phone number<input value={form.phone} onChange={(event) => set('phone', event.target.value)} required /></label><label>Email <span>(optional)</span><input type="email" value={form.email} onChange={(event) => set('email', event.target.value)} /></label>{mode === 'visit' && <label>Preferred date and time<input type="datetime-local" value={form.preferred_datetime} min={new Date().toISOString().slice(0, 16)} onChange={(event) => set('preferred_datetime', event.target.value)} required /></label>}<label>Message <span>(optional)</span><textarea rows="3" value={form.message} onChange={(event) => set('message', event.target.value)} /></label><FieldError error={state.error} /><button className="button button-primary" disabled={state.status === 'submitting'}>{mode === 'visit' ? <CalendarDays /> : <ExternalLink />}{state.status === 'submitting' ? 'Sending…' : mode === 'visit' ? 'Request site visit' : 'Send inquiry'}</button></form>
    </>}
